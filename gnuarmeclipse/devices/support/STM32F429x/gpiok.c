/*
 * STM32 - GPIOK (General-purpose I/Os) emulation.
 *
...
#include <hw/cortexm/stm32/gpiok.h>
...
static void stm32f429x_gpiok_create_objects(Object *obj, JSON_Object *svd, const char *name)
{
    STM32GPIOKState *state = STM32_GPIOK_STATE(obj);
...
#if 0
static peripheral_register_t stm32_gpiok_xxx_pre_write_callback(Object *reg,
        Object *periph, uint32_t addr, uint32_t offset, unsigned size,
        peripheral_register_t value, peripheral_register_t full_value)
{
    STM32GPIOKState *state = STM32_GPIOK_STATE(periph);
...
static void stm32_gpiok_xxx_post_write_callback(Object *reg, Object *periph,
...
static peripheral_register_t stm32_gpiok_xxx_pre_read_callback(Object *reg,
        Object *periph, uint32_t addr, uint32_t offset, unsigned size)
{
    STM32GPIOKState *state = STM32_GPIOK_STATE(periph);
...
static void stm32_gpiok_xxx_post_read_callback(Object *reg, Object *periph,
        uint32_t addr, uint32_t offset, unsigned size)
{
    STM32GPIOKState *state = STM32_GPIOK_STATE(periph);
...
static bool stm32_gpiok_is_enabled(Object *obj)
{
    STM32GPIOKState *state = STM32_GPIOK_STATE(obj);
...
static void stm32_gpiok_instance_init_callback(Object *obj)
{
...
static void stm32_gpiok_realize_callback(DeviceState *dev, Error **errp)
{
    qemu_log_function_name();

    // Call parent realize().
    if (!cm_device_parent_realize(dev, errp, TYPE_STM32_GPIOK)) {
        return;
    }
    ...
    STM32GPIOKState *state = STM32_GPIOK_STATE(dev);
    ...
    case STM32_FAMILY_F4:
        ...
        if (capabilities->f4.is_429x ) {
            ...
            stm32f429x_gpiok_create_objects(obj, cm_state->svd_json, periph_name);
            ...
            // TODO: add callbacks.
            // peripheral_register_set_pre_read(state->f4.reg.xxx, &stm32_gpiok_xxx_pre_read_callback);
            // peripheral_register_set_post_read(state->f4.reg.xxx, &stm32_gpiok_xxx_post_read_callback);
            // peripheral_register_set_pre_read(state->f4.reg.xxx, &stm32_gpiok_xxx_pret_read_callback);
            // peripheral_register_set_post_write(state->f4.reg.xxx, &stm32_gpiok_xxx_post_write_callback);

            // TODO: add interrupts.

            // TODO: remove this if the peripheral is always enabled.
            snprintf(enabling_bit_name, sizeof(enabling_bit_name) - 1,
                DEVICE_PATH_STM32_RCC "/AHB1ENR/GPIOKEN");
...
static void stm32_gpiok_reset_callback(DeviceState *dev)
{
    qemu_log_function_name();

    // Call parent reset(); this will reset all children registers.
    cm_device_parent_reset(dev, TYPE_STM32_GPIOK);
}

static void stm32_gpiok_class_init_callback(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);

    dc->reset = stm32_gpiok_reset_callback;
    dc->realize = stm32_gpiok_realize_callback;

    // TODO: remove this if the peripheral is always enabled.
    PeripheralClass *per_class = PERIPHERAL_CLASS(klass);
    per_class->is_enabled = stm32_gpiok_is_enabled;
}

static const TypeInfo stm32_gpiok_type_info = {
    .name = TYPE_STM32_GPIOK,
    .parent = TYPE_STM32_GPIOK_PARENT,
    .instance_init = stm32_gpiok_instance_init_callback,
    .instance_size = sizeof(STM32GPIOKState),
    .class_init = stm32_gpiok_class_init_callback,
    .class_size = sizeof(STM32GPIOKClass) };

static void stm32_gpiok_register_types(void)
{
    type_register_static(&stm32_gpiok_type_info);
}

type_init(stm32_gpiok_register_types);
